...
CREATE TABLE public.datos_bancarios (
  ...
  CONSTRAINT datos_bancarios_dba_tcb_id_fkey FOREIGN KEY (dba_tcb_id) REFERENCES public.tipo_cuenta_bancaria(tcb_id)
);
...
CREATE TABLE public.personal (
  ...
  per_telefono_llamada character varying NOT NULL,
  ...
  per_talla_pantalon character varying NOT NULL,
  per_talla_zapatos character varying NOT NULL,
...
CREATE TABLE public.tipo_cuenta_bancaria (
  tcb_id integer NOT NULL DEFAULT nextval('tipo_cuenta_bancaria_tcb_id_seq'::regclass),
  tcb_tipo_cuenta character varying NOT NULL UNIQUE,
  created_at timestamp without time zone DEFAULT now(),
  updated_at timestamp without time zone,
  CONSTRAINT tipo_cuenta_bancaria_pkey PRIMARY KEY (tcb_id)
);